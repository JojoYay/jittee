...
| SKU | Price | Includes |
|-----|-------|----------|
| `minorwire_app` | SGD 18 | DIY: IAM guide + provision flow |
| `minorwire_support` | SGD 100 | Live support (WhatsApp screen-share); buy alone after DIY or with DIY via bundle checkout |

Currency: **SGD**. Two Stripe products only. Bundle Payment Link sells both in one checkout (S$118 total). Legacy `minorwire_setup` is no longer sold.
...
### DIY + support bundle (S$118)

Same as buying `minorwire_app` + `minorwire_support` together (one Payment Link, two Stripe products).

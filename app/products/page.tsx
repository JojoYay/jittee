...
// 製品カード (Splitator / SpotMyShot / Local Recorder など)。各詳細ページへ遷移
const APPS: Record<string, { name: string; href: string; img: string; desc: string }[]> = {
  ja: [
    { name: 'Splitator', href: '/splitbill', img: '/splitbill/icon-512.png', desc: '立て替えた分をPayNowで集める割り勘ツール。登録不要・URLを配るだけ。' },
  ...
  en: [
    { name: 'Splitator', href: '/splitbill', img: '/splitbill/icon-512.png', desc: 'Collect what you paid up front, over PayNow. No sign-up — just share a link.' },
  ...
  zh: [
    { name: 'Splitator', href: '/splitbill', img: '/splitbill/icon-512.png', desc: '用 PayNow 收回先垫付的钱。免注册，分享连结即可。' },
          ...
          {apps.map((a) => (
            <Link key={a.href} href={a.href} className="group bg-white rounded-xl shadow p-6 flex items-start gap-4 hover:shadow-lg transition-shadow">
              {a.img ? (
                ...
                <div className="w-14 h-14 rounded-xl flex-shrink-0 bg-[#1d3d2e] text-[#9fd6b8] flex items-center justify-center font-bold text-lg">
                  MW
                </div>
              )}
              <div>
